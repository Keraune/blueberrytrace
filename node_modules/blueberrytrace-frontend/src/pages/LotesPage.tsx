...
export function LotesPage({ lotes, camas, siembras, onLotesChange }: LotesPageProps) {
  ...
  const cards = [
    ...
    { label: 'En Proceso', value: lotes.filter((item) => /PROCESO|MANTENIMIENTO/i.test(item.estado || '')).length, tone: 'blue', icon: <ClipboardList size={18} /> },
    { label: 'Cosecha', value: lotes.filter((item) => /COSECHA/i.test(item.estado || '')).length, tone: 'purple', icon: <PackageCheck size={18} /> },
    { label: 'Pendientes', value: lotes.filter((item) => /PENDIENTE/i.test(item.estado || '')).length, tone: 'orange', icon: <Clock3 size={18} /> }
  ];
